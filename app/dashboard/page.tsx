...
import { QrCode, Home, Settings, Download, 
  ArrowUpDown, Building2, Edit, Trash2, Clock, AlertTriangle, Info,
  Printer, MapPin, Utensils, MoreHorizontal, ExternalLink, Wrench, ChevronRight, ChevronLeft, X, Zap,
  Book, Wifi, Camera, Phone, CalendarCheck, CheckCircle, HelpCircle } from 'lucide-react'
import {
DropdownMenu,
DropdownMenuContent,
DropdownMenuItem,
...
export default function StayScanDashboard() {
  ...
  const [newProperty, setNewProperty] = useState<Omit<Property, 'id'> & { image: File | null }>({ 
    name: "", 
    location: "", 
    description: "", 
    amenities: "", 
  ...
  const [currentSubscription, setCurrentSubscription] = useState<{name: string} | null>(null)
  const [isAddPropertyDialogOpen, setIsAddPropertyDialogOpen] = useState(false)
  const [language, setLanguage] = useState('en')
  const [addPropertyStep, setAddPropertyStep] = useState(1)
  const [activePropertyCard, setActivePropertyCard] = useState<string | null>(null)
  const [showUpgradeBox, setShowUpgradeBox] = useState(true)
  const [showSubscriptionDialog, setShowSubscriptionDialog] = useState(false)
  const [isSidebarExpanded, setIsSidebarExpanded] = useState(true)
  const [uploadingImage, setUploadingImage] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')
  const [sortColumn, setSortColumn] = useState<'property' | 'status' | 'title' | 'issue'>('property')
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc')
  const [selectedPropertyForArchive, setSelectedPropertyForArchive] = useState('')
  ...
  const sortIssues = useCallback((issues: MaintenanceIssue[]) => {
  ...
  const filterIssues = useCallback((issues: MaintenanceIssue[]) => {
    return issues.filter(issue => 
      ((issue.property?.name || '').toLowerCase().includes(searchTerm.toLowerCase()) || false) ||
      issue.issue.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [searchTerm])

  const sortedActiveIssues = useMemo(() => sortIssues(filterIssues(maintenanceIssues)), [maintenanceIssues, filterIssues, sortIssues])
  const sortedArchivedIssues = useMemo(() => {
    const filteredIssues = archivedIssues.filter(issue => 
      selectedPropertyForArchive ? issue.property?.name === selectedPropertyForArchive : true
    )
 ...
 // Effect hooks
  ...
  useEffect(() => {
    if (currentSubscription?.name && currentSubscription.name !== "Enterprise") {
  ...
  const renderAddPropertyStep = () => {
    switch (addPropertyStep) {
        ...
        return (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-sm font-medium text-emerald-700">Property Name</Label>
              <Input
                id="name"
            ...
            <div className="space-y-2">
              <Label htmlFor="location" className="text-sm font-medium text-emerald-700">Location</Label>
              <Input
                id="location"
      ...
      case 2:
        const cards = [
          { id: 'houseRules', title: 'House Rules', placeholder: 'e.g. No smoking, Quiet hours after 10 PM', icon: <Book className="h-4 w-4" /> },
          { id: 'nearbyPlaces', title: 'Local Attractions', placeholder: 'e.g. Beach, Restaurants, Shopping', icon: <Camera className="h-4 w-4" /> },
          { id: 'emergencyContact', title: 'Emergency Contact & Other Contacts', placeholder: 'e.g. +1 (555) 123-4567', icon: <Phone className="h-4 w-4" /> },
          { id: 'rubbishAndBins', title: 'Rubbish & Bins', placeholder: 'e.g. Collection days, recycling instructions', icon: <Trash2 className="h-4 w-4" /> },
          { id: 'applianceGuides', title: 'Appliance Guides', placeholder: 'Information for different kind of guides', icon: <Trash2 className="h-4 w-4" /> },
          { id: 'wifi', title: 'WiFi', placeholder: 'e.g. Network Name: MyWiFi, Password: 12345', icon: <Wifi className="h-4 w-4" /> },
          { id: 'localFood', title: 'Local Food', placeholder: 'e.g. Italian: Luigis Pizzeria, Sushi: Tokyo Diner', icon: <Utensils className="h-4 w-4" /> },
          { id: 'checkOutDay', title: 'Check-out Day', placeholder: 'e.g. Sunday at 11:00 AM', icon: <CalendarCheck className="h-4 w-4" /> },
        ]
          ...
          <div className="space-y-4 max-h-[calc(100vh-8rem)] overflow-y-auto p-4">
            <div className="flex items-center space-x-2 mb-4">
              <Button 
                ...
                disabled={isGeneratingAI || !newProperty.name || !newProperty.location}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" size="icon" className="h-8 w-8 text-white bg-emerald-600 hover:bg-emerald-700">
                ...
                <PopoverContent className="w-80">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">We&apos;re thrilled to have you onboard. Thanks for choosing us to make your hosting life a little easier (and more fun)!</p>
                    <p className="text-sm text-muted-foreground">Before adding your first property, here are two key things to know:</p>
                    <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                      <li><strong>AI-Powered Assistance:</strong> By selecting the &quot;Generate with AI&quot; button, we&apos;ll create basic information for you based on the details we have. We recommend reviewing and customizing it to ensure accuracy and suit your specific listing.</li>
                      <li><strong>Easy Customization:</strong> You can easily edit each field by tapping on the card and entering your information. Get started now and make your listing shine!</li>
                    </ul>
                  </div>
                </PopoverContent>
              </Popover>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {cards.map((card) => (
                <Card 
                  key={card.id} 
                  className={`cursor-pointer transition-all hover:shadow-lg ${activePropertyCard === card.id ? 'ring-2 ring-emerald-500' : ''}`}
                ...
                >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 py-2">
                    ...
                    {newProperty[card.id as keyof typeof newProperty] && <CheckCircle className="h-4 w-4 text-emerald-500" />}
                  </CardHeader>
                  <CardContent className="py-2">
                    <Textarea
                      id={card.id}
                      value={newProperty[card.id as keyof typeof newProperty] as string}
                      onChange={(e) => setNewProperty({ ...newProperty, [card.id]: e.target.value })}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 gap-3">
              <Card>
                <CardHeader className="py-2">
                  <CardTitle className="text-xs font-medium">Property Image</CardTitle>
                </CardHeader>
                <CardContent className="py-2">
                  <Input
                    id="image"
      ...
      case 3:
        return (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="description" className="text-sm font-medium text-emerald-700">Welcome Message / Description</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="digitalGuide" className="text-sm font-medium text-emerald-700">Digital Guide</Label>
              <div className="flex space-x-2">
                <Textarea
                  id="digitalGuide"
                  name="digitalGuide"
                  ...
                  onChange={(e) => {
                    ...
                    if (words.length <= 200) {
                      setNewProperty({ ...newProperty, digitalGuide: e.target.value });
                    }
                  }}
                  className="flex-grow h-32 border-emerald-300 focus:border-emerald-500 focus:ring-emerald-500"
  ...
  const createOrUpdateUser = useCallback(async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to update user information. Some features may be limited.",
        variant: "destructive",
      })
    }
  }, [user?.id, toast])
  ...
  const generateDescription = async () => {
    ...
    try {
      ...
      const data = await response.json()
      setNewProperty(prev => ({ ...prev, description: data.generatedDescription }))
      toast({
        title: "Success",
  ...
  const getFirstImage = (images: string | string[] | null): string => {
    ...
    if (typeof images === 'string') {
      ...
      return imageArray.length > 0 && imageArray[0].trim() !== '' ? imageArray[0].trim() : '/placeholder.jpg'
    }
    if (Array.isArray(images)) {
      return images.length > 0 && images[0].trim() !== '' ? images[0].trim() : '/placeholder.jpg'
    }
  ...
  const generateDigitalGuide = async () => {
    ...
    try {
      ...
      const data = await response.json()
      setNewProperty(prev => ({ ...prev, digitalGuide: data.generatedGuide }))
      toast({
        title: "Success",
  ...
  const fetchSubscription = useCallback(async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch subscription data. Some features may be limited.",
        variant: "destructive",
      })
    }
  }, [toast])
  ...
  }, [createOrUpdateUser, fetchProperties, fetchMaintenanceIssues, fetchSubscription])
  ...
  const addProperty = async () => {
    if (newProperty.name && newProperty.location && subscription && properties.length < subscription.propertyLimit) {
      try {
        setUploadingImage(true)
        let imageUrl = ''
  
        if (newProperty.image) {
          const imageBlob = await put(newProperty.image.name, newProperty.image, {
            access: 'public',
        ...
        setNewProperty({ 
          name: "", location: "", description: "", wifi: "", localFood: "", applianceGuides: "", rubbishAndBins: "", amenities: "", 
          images: "", status: 'inactive', qrScans: 0, image: null,
          emergencyContact: "", nearbyPlaces: "", checkOutDay: "", houseRules: "", digitalGuide: "",
        })
        toast({
          title: "Success",
    ...
    } else if (subscription && properties.length >= subscription.propertyLimit) {
      toast({
        title: "Property limit reached",
        description: "You've reached your property limit. Please upgrade your subscription to add more properties.",
        variant: "destructive",
      })
    } else {
      toast({
        title: "Incomplete information",
        description: "Please fill in all required fields (name, location, price).",
        variant: "destructive",
      })
    }
  }
  ...
  const updateMaintenanceStatus = async (issueId: string, newStatus: string) => {
    try {
      ...
      if (newStatus === 'Resolved') {
        setMaintenanceIssues(prevIssues => prevIssues.filter(issue => issue.id !== issueId))
        setArchivedIssues(prevIssues => [...prevIssues, updatedIssue])
      } else {
        setMaintenanceIssues(prevIssues => prevIssues.map(issue => issue.id === issueId ? updatedIssue : issue))
      }
    } catch (error) {
  ...
  const tabItems = [
    ...
    { value: "amenities", icon: <Utensils className="w-4 h-4" />, label: "Amenities" },
    { value: "rules", icon: <Info className="w-4 h-4" />, label: "Rules" },
    { value: "nearby", icon: <MapPin className="w-4 h-4" />, label: "Nearby" },
    { value: "additional", icon: <Wifi className="w-4 h-4" />, label: "Additional" },
  ]
  ...
  const getStatusVariant = (status: string): "destructive" | "default" | "outline" => {
    switch (status) {
      case 'Open':
        return 'destructive'
        case 'In Progress':
          ...
          const QRCodeDialog = ({ propertyId }: { propertyId: string | null }) => {
           ...
           const handlePrint = () => {
            ...
            const handleDownload = () => {
              ...
              link.download = `${property.name.replace(/\s+/g, '-').toLowerCase()}-qr-code.png`
    ...
    return (
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-emerald-700">QR Code for {property.name}</DialogTitle>
        ...
        <div className="flex flex-col items-center space-y-6">
          <div className="w-64 h-64 bg-emerald-50 flex items-center justify-center rounded-lg border-2 border-emerald-200 shadow-lg">
            {qrCodeData && <img src={qrCodeData} alt={`QR Code for ${property.name}`} className="w-full h-full" />}
          </div>
          <div className="flex space-x-4">
            ...
            <Button onClick={handleDownload} variant="outline" className="border-emerald-600 text-emerald-600 hover:bg-emerald-50">
  ...
  const MaintenanceTable = ({ issues, isArchived }: { issues: MaintenanceIssue[], isArchived: boolean }) => (
    <div className="space-y-4">
...
>
                  <span className="font-semibold text-emerald-800">Property</span>
          ...
          <TableBody>
            {issues.map((issue) => (
              ...
              >
                ...
                <Badge
  variant={issue.status === 'Open' ? 'destructive' : issue.status === 'In Progress' ? 'default' : 'outline'}
...
>
                ...
                <TableCell className="py-4 px-4">
                  <div className="flex items-center space-x-2">
                    ...
                    {!isArchived && (
                      <Select
                        value={issue.status}
                        onValueChange={(value) => updateMaintenanceStatus(issue.id, value)}
                      >
  ...
  return (
    <div className={`flex h-screen bg-gradient-to-br from-${colors.oxfordBlue} to-${colors.black} ${isDarkMode ? 'dark' : ''}`} style={{ fontFamily: 'SF Pro Display, sans-serif' }}>
      {/* Maintenance Issue Details Dialog */}
      <Dialog open={!!selectedIssue} onOpenChange={() => setSelectedIssue(null)}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-emerald-700">Maintenance Issue Details</DialogTitle>
          </DialogHeader>
          {selectedIssue && (
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <p>{selectedIssue.property ? selectedIssue.property.name : 'Unknown Property'}</p>
              </div>
              <div>
...
>
  ...
  <span>{selectedIssue.status}</span>
                ...
                <Tooltip key={tab}>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={`w-full justify-${isSidebarExpanded ? 'start' : 'center'} rounded-xl py-6 hover:bg-[#1e3932] hover:bg-opacity-70 text-[#d4e9e2] hover:text-[#dff9ba] transition-all duration-200 ${activeTab === tab ? 'bg-[#1e3932] bg-opacity-70 text-[#dff9ba] shadow-lg' : ''}`}
                  ...
                  {!isSidebarExpanded && <TooltipContent side="right" className="capitalize">{tab}</TooltipContent>}
                </Tooltip>
              ))}
            </nav>
            {/* Sidebar Footer */}
              ...
              >
                ...
                {isSidebarExpanded && user && (
                  <motion.div 
                    ...
                    className="ml-3"
                  >
                    <p className="text-sm font-medium text-[#d4e9e2]">{user.fullName}</p>
                    <p className="text-xs text-[#dff9ba]">{user.primaryEmailAddress?.emailAddress}</p>
                  </motion.div>
                )}
              </motion.div>
            </div>
                ...
                <div className="mb-8 flex justify-between items-center">
                  ...
                  <Dialog open={isAddPropertyDialogOpen} onOpenChange={setIsAddPropertyDialogOpen}>
                    <DialogTrigger asChild>
                      <Button 
                        className="mb-4" 
                        disabled={properties.length >= (subscription?.propertyLimit ?? 0)}
                    ...
                    <DialogContent className="sm:max-w-[900px]">
                      <DialogHeader>
                        <DialogTitle className="text-2xl font-semibold text-emerald-800">Add New Property</DialogTitle>
                        <DialogDescription className="text-emerald-600">
                          Enter the details of your new property below. Step {addPropertyStep} of 3
                        </DialogDescription>
                      </DialogHeader>
                      ...
                      <DialogFooter>
                        {addPropertyStep > 1 && (
                          <Button 
                            variant="outline" 
                            onClick={() => setAddPropertyStep(addPropertyStep - 1)}
                        ...
                        {addPropertyStep < 3 ? (
                          <Button 
                            onClick={() => setAddPropertyStep(addPropertyStep + 1)}
                            className="bg-emerald-600 hover:bg-emerald-700 text-white"
                            disabled={addPropertyStep === 1 && (!newProperty.name || !newProperty.location)}
                          >
                            Continue
                          </Button>
                        ) : (
                          <Button 
                            onClick={addProperty} 
                            disabled={uploadingImage || !newProperty.name || !newProperty.location || !newProperty.description} 
                            className="bg-emerald-600 hover:bg-emerald-700 text-white"
                          >
                    ...
                    {properties.map((property) => (
                        ...
                        <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                          ...
                          <CardContent className="p-4">
                            <div className="flex justify-between items-center mb-3">
                              ...
                              <span className="text-sm text-gray-500">{property.qrScans} scans</span>
                            </div>
                      ...
                      <MoreHorizontal className="h-5 w-5" />
                                ...
                                <DropdownMenuContent align="end" className="w-48">
                                  ...
                                  <DropdownMenuItem onClick={() => handleQRCode(property.id)}>
      ...
      <span>View Page</span>
                                  ...
                                  <DropdownMenuItem onClick={() => deleteProperty(property.id)}>
              ...
              >
                <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 shadow-lg">
                  <CardHeader className="border-b border-emerald-100">
                    <CardTitle className="text-2xl font-bold text-emerald-800">Maintenance Issues</CardTitle>
                  ...
                  <CardContent className="p-6">
                    <Tabs defaultValue="active" className="w-full">
                      <TabsList className="grid w-full grid-cols-2 mb-6">
                        <TabsTrigger value="active" className="text-emerald-700">Active Issues</TabsTrigger>
                        <TabsTrigger value="archived" className="text-emerald-700">Archived Issues</TabsTrigger>
                      </TabsList>
                      <TabsContent value="active">
                        <MaintenanceTable issues={sortedActiveIssues} isArchived={false} />
                      </TabsContent>
                      <TabsContent value="archived">
                        <MaintenanceTable issues={sortedArchivedIssues} isArchived={true} />
                      </TabsContent>
                    </Tabs>
                  </CardContent>
                </Card>
            ...
            {activeTab === "settings" && (
              ...
              >
                <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-2xl font-bold text-emerald-800">Account Settings</CardTitle>
...
<AvatarImage src={user?.imageUrl} alt={user?.fullName ?? 'User avatar'} />
            ...
            <div>
              ...
              <p className="text-gray-500">{user?.primaryEmailAddress?.emailAddress}</p>
                    ...
                    <div className="space-y-2">
                      <h3 className="text-lg font-semibold text-emerald-700">Subscription Information</h3>
                      {subscription ? (
                        ...
                        <p className="text-emerald-600">No active subscription found.</p>
                      )}
                      <Button onClick={() => setShowSubscriptionDialog(true)} className="mt-4 bg-emerald-600 hover:bg-emerald-700 text-white">
                        Manage Subscription
                      </Button>
                    </div>
      ...
      <Dialog open={showSubscriptionDialog} onOpenChange={setShowSubscriptionDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-emerald-700">Manage Subscription</DialogTitle>
            <DialogDescription className="text-emerald-600">
          ...
          <div className="grid gap-4 py-4">
            {subscriptionPlans.map((plan) => (
              <Card key={plan.name} className={`${currentSubscription?.name === plan.name ? 'border-emerald-500' : 'border-gray-200'} transition-all duration-300 hover:shadow-md`}>
                <CardHeader>
                  <CardTitle className="text-xl font-semibold text-emerald-700">{plan.name}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-3xl font-bold text-emerald-600">${plan.price}<span className="text-base font-normal text-gray-500">/month</span></p>
                  <p className="mt-2 text-gray-600">Up to {plan.propertyLimit} properties</p>
                </CardContent>
                <CardFooter>
                  {currentSubscription?.name === plan.name ? (
                    <Button disabled className="w-full bg-emerald-100 text-emerald-700">Current Plan</Button>
                  ) : (
<Button onClick={() => handleSubscription(plan)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white">
  {currentSubscription ? 'Switch to this plan' : 'Select this plan'}
</Button>
                  )}
                </CardFooter>
              </Card>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-emerald-700">Edit Property</DialogTitle>
          <DialogDescription className="text-emerald-600">
        ...
        {selectedProperty && (
              ...
              {tabItems.map((item) => (
                <TabsTrigger
                  ...
                  className={`flex items-center justify-center space-x-2 ${
                    activeEditTab === item.value ? 'bg-emerald-100 text-emerald-700' : 'text-gray-600'
                  }`}
                >
                  {item.icon}
          ...
          <TabsContent value="amenities">
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="edit-amenities" className="text-sm font-medium text-gray-700">
                  Amenities
                </Label>
                <Textarea
                  id="edit-amenities"
          ...
          <TabsContent value="rules">
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="edit-rules" className="text-sm font-medium text-gray-700">
                  House Rules
                </Label>
                <Textarea
                  id="edit-rules"
                  value={selectedProperty.houseRules}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, houseRules: e.target.value })}
                  className="w-full"
          ...
          <TabsContent value="nearby">
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="edit-nearby" className="text-sm font-medium text-gray-700">
                  Nearby Attractions
                </Label>
                <Textarea
                  id="edit-nearby"
                  value={selectedProperty.nearbyPlaces}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, nearbyPlaces: e.target.value })}
                  className="w-full"
          ...
          <TabsContent value="additional">
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="edit-wifi" className="text-sm font-medium text-gray-700">
                  WiFi Information
                </Label>
                <Textarea
                  id="edit-wifi"
                  value={selectedProperty.wifi}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, wifi: e.target.value })}
                  className="w-full"
                  placeholder="e.g. Network Name: MyWiFi, Password: 12345"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-emergency" className="text-sm font-medium text-gray-700">
                  Emergency Contact & Other Contacts
                </Label>
                <Input
                  id="edit-emergency"
                  value={selectedProperty.emergencyContact}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, emergencyContact: e.target.value })}
                  className="w-full"
                  placeholder="e.g. +1 (555) 123-4567"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-rubbish" className="text-sm font-medium text-gray-700">
                  Rubbish And Bins
                </Label>
                <Textarea
                  id="edit-rubbishAndBins"
                  value={selectedProperty.rubbishAndBins}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, rubbishAndBins: e.target.value })}
                  className="w-full"
                  placeholder="e.g. Collection days, recycling instructions"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-local-food" className="text-sm font-medium text-gray-700">
                  Local Food
                </Label>
                <Textarea
                  id="edit-local-food"
                  value={selectedProperty.localFood}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, localFood: e.target.value })}
                  className="w-full"
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-checkout" className="text-sm font-medium text-gray-700">
                  Check-out Day
                </Label>
                <Input
                  id="edit-checkout"
                  value={selectedProperty.checkOutDay}
                  onChange={(e) => setSelectedProperty({ ...selectedProperty, checkOutDay: e.target.value })}
                  className="w-full"
      ...
      <Dialog open={showUpgradeDialog} onOpenChange={setShowUpgradeDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-emerald-700">Upgrade Your Subscription</DialogTitle>
            <DialogDescription className="text-emerald-600">
...
<Button onClick={() => handleSubscription(plan)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white">
  Upgrade to {plan.name}
      ...
      {showUpgradeBox && currentSubscription && currentSubscription.name !== "Enterprise" && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <p className="text-sm mb-3 text-white/90">Unlock unlimited properties and premium features.</p>
          <Button 
            onClick={() => setShowSubscriptionDialog(true)} 
            className="w-full bg-white text-[#086241] hover:bg-green-50 transition-colors"
            variant="outline"
          >
        ...
        <div className="fixed bottom-4 right-4 z-50">
          <Alert variant="destructive">
            ...
            <AlertDescription className="text-red-700">
              It appears that Stripe is being blocked. This may interfere with subscription management. Please disable any ad blockers or security extensions for this site.
            </AlertDescription>
          </Alert>
        </div>
      )}
